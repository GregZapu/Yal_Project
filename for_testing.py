        ...
        if movment_type == pygame.KEYDOWN:
            ...
            if movement_button == pygame.K_e:
                ...
                if current_location == "store":
                    ...
                    if self.player_coords[0] >= 150 and self.player_coords[0] <= 650:
                        if self.player_coords[1] >= 100 and self.player_coords[1] <= 155:
                            if coins >= 1:
                                ...
                                cur.execute("""UPDATE Store
                                    SET count = count + 1
                                    WHERE upgrade_name == 'speed_upgrade'""")
                                con.commit()
                    if self.player_coords[0] >= 150 and self.player_coords[0] <= 650:
            ...
            if movement_button == pygame.K_d:
                movement_speed[0] -= 5
                self.way[0] -= 1
            if movement_button == pygame.K_SPACE and self.player_dash == 1:
                self.aim_dash = False
                for i in range(1, 14):
    ...
    def render(self, screen):
        ...
        if self.lifetime == 0:
            if self.bullet_owner == modul2:
                ...
                self.bullet_appear = False
        if (self.current_pos[0] - modul2.module_coords[0] >= -38) and (self.current_pos[0] - modul2.module_coords[0]) <= 38 and self.bullet_owner == player and module_render2 == True:
        ...
        if (self.current_pos[0] - modul1.module_coords[0] >= -38) and (self.current_pos[0] - modul1.module_coords[0]) <= 38 and self.bullet_owner == player and module_render1 == True:
        ...
        if (self.current_pos[0] < 0) or (self.current_pos[0] > 800) and self.bullet_owner == player and player.bouncing:
            self.x_inert *= -1
            self.an = - self.an

        if (self.current_pos[1] < 0) or (self.current_pos[1] > 800) and self.bullet_owner == player and player.bouncing:
            self.y_inert *= -1
...
def store_location():
    ...
    Speed_upgrade_button.render(screen)
...
def battle_field():
    ...
    if module_render1 and boss_render == True and player.player_dash == 1:
        module_healthbar1.render(screen, modul1.module_coords)
        modul1.module_movement()
        modul1.module_render(screen)
    if module_render2 and boss_render == True and player.homing == 1:
...
if __name__ == '__main__': 
    ...
    Speed_upgrade_button = Button("Speed upgrade - 1 coin", [500, 55], [150, 100], 54, 10)
    ...
    module1_left.image = pygame.image.load("Sprite_module_left_for_pj.png")
    module1_right.image = pygame.image.load("Sprite_module_right_for_pj.png")
    module2_left.image = pygame.image.load("Sprite_module2_left_for_pj.png")
    module2_right.image = pygame.image.load("Sprite_module2_right_for_pj.png")
